...
TITLE = [
    ...
    # handles the rest & recreation popup in summer # TITLE[2]
    "Rest & Recreation",
    "Network Error",                  # TITLE[3]
    # handles the try again button when race failed (used clock items) # TITLE[4]
    "Try Again",
    "Earned Title",                  # TITLE[5]
    "Training Complete",              # TITLE[6]
    # handles the quick mode settings popup dialog # TITLE[7]
    "Quick Mode Settings",
    "Recreation",  # handles the recreation popup dialog # TITLE[8]
    # handles the initial confirmation when learning skill starts # TITLE[9]
    "Confirmation",
    # handles the final completion dialog after learning skills is done # TITLE[10]
    "Skills Learned",
    "Complete Career",  # handles the complete career popup dialog # TITLE[11]
    # handles the umamusume details after career finish popup dialog # TITLE[12]
    "Umamusume Details",
    "Fan Count Below Target Race Requirement",  # TITLE[13]
    "Outing",                        # TITLE[14]
    # handles the skip confirmation content when first start career # TITLE[15]
    "Skip Confirmation",
    "Rest",  # handles the rest popup confirmationdialog # TITLE[16]
    # handles the race recommendation popup dialog # TITLE[17]
    "Race Recommendations",
    "Tactics",  # fallback to strategy # TITLE[18]
    "Strategy",  # handles the strategy change popup in race # TITLE[19]
    # handles the goal not reached popup dialog (Oguri Cap G1 Goals) # TITLE[20]
    "Goal Not Reached",
    # maybe the right insufficient fans dialog???????????????### # TITLE[21]
    "Insufficient Fans",
    # handles the warning 3 consecutive racing popup dialog # TITLE[22]
    "Warning",
    "Infirmary",  # handles the infirmary popup confirmation dialog # TITLE[23]
    "Gift Box",                      # TITLE[24]
    # maybe event collection successful??????????????### # TITLE[25]
    "Collection Successful",
    "Character Story Unlocked",      # TITLE[26]
    "Skill Acquisition Confirmation",  # TITLE[27]
    "Successfully Acquired Skill",   # TITLE[28]
    # TITLE[29] - FIXED: was "Confirm"
    "Target Achievement Count Insufficient",
    "Event Story Unlocked",          # TITLE[30]
    # Recover TP Confirm button if you enable auto recover tp in UAT website # TITLE[31] - FIXED: was "Target Achievement Count Insufficient"
    "Confirm",
    # Recover TP Confirm popup dialog to buy the recover tp item # TITLE[32]
    "Recover TP",
...
def script_info(ctx: UmamusumeContext):
    ...
    if result.find_match:
        ...
        title_img = img[pos[0][1] - 5:pos[1][1] +
                        5, pos[0][0] + 150: pos[1][0] + 405]
        ...
        # --- NEW: Fast bypass for Difficulty Select (Golshi Mode / Events) ---
        if "difficulty" in original_text.lower() or "playthrough" in original_text.lower():
            log.info(
                "Career Difficulty Select detected. Selecting event mode and clicking Confirm.")
            time.sleep(0.8)  # Delay to let UI animation settle
            # Click the second option (Special Event Mode / Golshi Mode)
            ctx.ctrl.click(360, 450, "Select Special Event Mode")
            time.sleep(0.5)
        ...
        if title_text in (TITLE[55], TITLE[56], TITLE[57], TITLE[58]):
            ctx.ctrl.click(360, 1180, "Close Campaign/Notice popup")
            time.sleep(0.5)
            return
        ...
        if title_text == TITLE[4]:  # Try Again
            ...
            if result.find_match:
                ...
                if ctx.cultivate_detail.clock_used < ctx.cultivate_detail.clock_use_limit:
                    ...
                    log.info("Clock limit %s, used %s", str(
                        ctx.cultivate_detail.clock_use_limit), str(ctx.cultivate_detail.clock_used))
                else:
                ...
                if retry_result.find_match:
                    if ctx.cultivate_detail.clock_used < ctx.cultivate_detail.clock_use_limit:
                        ctx.ctrl.click_by_point(RACE_FAIL_CONTINUE_USE_CLOCK)
                        ctx.cultivate_detail.clock_used += 1
                        log.info("(retry) Clock limit %s, used %s", str(
                            ctx.cultivate_detail.clock_use_limit), str(ctx.cultivate_detail.clock_used))
                    else:
                        ctx.ctrl.click_by_point(RACE_FAIL_CONTINUE_CANCEL)
        ...
        if title_text == TITLE[43]:
            ...
            matching_races = [
                race_id for race_id in next_period_races] if next_period_races else []
            matching_races = [
                race_id for race_id in matching_races if race_id in user_selected_races]
            ...
            if not matching_races:
                ...
                if suitable_race_match.find_match:
                    ...
                    log.info(
                        f"Clicked suitable race at position ({center_x}, {center_y})")
                else:
                    log.info(
                        "Suitable race template not found - returning to main menu for normal logic")
        ...
        if title_text == TITLE[15]:  # Skip Confirmation
            ...
            log.info(
                f"Skip Confirmation - Template matching result: {result.find_match}")
            if result.find_match:
                # Use template matching coordinates for skill confirmation
                center_x = (
                    result.matched_area[0][0] + result.matched_area[1][0]) // 2
                center_y = (
                    result.matched_area[0][1] + result.matched_area[1][1]) // 2
                ctx.ctrl.click(
                    center_x, center_y, "Skill confirmation using template matching (Skip Confirmation)")
                log.info(
                    f"Found skill confirmation button at ({center_x}, {center_y}) via Skip Confirmation")
            else:
                # Use CULTIVATE_LEARN_SKILL_CONFIRM_AGAIN coordinates for skill confirmations
                ctx.ctrl.click_by_point(CULTIVATE_LEARN_SKILL_CONFIRM_AGAIN)
                log.info(
                    "Using skill confirmation coordinates for Skip Confirmation - template not found")
        if title_text == TITLE[16]:  # Rest
            ctx.ctrl.click_by_point(CULTIVATE_OPERATION_COMMON_CONFIRM)
        if title_text == TITLE[17]:  # Race Recommendations
            ctx.ctrl.click_by_point(RACE_RECOMMEND_CONFIRM)
        # "Tactics" or "Strategy"
        if title_text == TITLE[18] or title_text == TITLE[19]:
            ...
            if date != -1:
                ...
                if target_tactic:
                    ...
                    ctx.ctrl.click_by_point(
                        TACTIC_LIST[ctx.cultivate_detail.tactic_list[2] - 1])
            time.sleep(0.5)
            ctx.ctrl.click_by_point(BEFORE_RACE_CHANGE_TACTIC_CONFIRM)
        # "Goal Not Reached" - Navigate to races to fulfill goal
        if title_text == TITLE[20]:
            # For Oguri Cap G1 race goals, go to race selection instead of failing
            log.info(
                "Goal Not Reached detected - navigating to races to fulfill G1 requirements")
            ...
            log.info(
                f"Current game date: {current_date} ({current_date_name}), planning races for: {next_period} ({next_period_name})")
            log.info(
                f"Updated game date to: {current_date} ({current_date_name})")
            ...
            if next_period_races:
                log.info(
                    f"Found {len(next_period_races)} races in next period {next_period} ({next_period_name}): {next_period_races}")

                # Check if any of these races are in user's selected race list
                user_selected_races = ctx.cultivate_detail.extra_race_list
                matching_races = [
                    race_id for race_id in next_period_races if race_id in user_selected_races]

                if matching_races:
                    log.info(
                        f"Found {len(matching_races)} user-selected races in next period {next_period} ({next_period_name}): {matching_races}")
                    # Set the first matching race as target
                    target_race_id = matching_races[0]
                    log.info(f"Setting target race ID: {target_race_id}")
                else:
                    log.info(
                        f"No user-selected races found in next period {next_period} ({next_period_name})")
                    target_race_id = 0  # Will search for any available race
            else:
                log.info(
                    f"No races available in next period {next_period} ({next_period_name})")
                target_race_id = 0  # Will search for any available race
            ...
            if not matching_races:
                log.info(
                    "No user-selected races found - searching for suitable race template")
        ...
        if title_text == TITLE[27]:  # Skill Acquisition Confirmation
            ...
            if result.find_match:
                ...
                center_y = (
                    result.matched_area[0][1] + result.matched_area[1][1]) // 2
                ctx.ctrl.click(center_x, center_y,
                               "Skill confirmation using template matching")
                log.info(
                    f"Found skill confirmation button at ({center_x}, {center_y})")
            else:
                # Fallback to hardcoded coordinates
                ctx.ctrl.click_by_point(CULTIVATE_LEARN_SKILL_CONFIRM_AGAIN)
                log.info(
                    "Using fallback coordinates for skill confirmation - template not found")
